"""Evaluation utilities for PyTorch CNN model on MNIST.
...
def compute_metrics(
    y_true: np.ndarray, y_pred: np.ndarray, y_logits: np.ndarray = None
    ...
    Args:
        ...
        y_logits: Predicted logits (optional, shape (n, 10), for reference)
...
def plot_history(
    ...
    Args:
        history: Dictionary with keys 'train_loss', 'train_accuracy', 'val_loss', 'val_accuracy'
                 (custom training loop format)
...
def infer_batch(model, batch_data: torch.Tensor, device: str = "cpu") -> np.ndarray:
    """
    Perform inference on a batch of images.

    Args:
        model: Trained PyTorch model
        batch_data: Batch of images (shape (batch_size, 1, 28, 28))
