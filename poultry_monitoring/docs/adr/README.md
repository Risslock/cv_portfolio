...
| [0017](0017-training-time-copy-paste-augmentation.md) | Copy-paste runs per-sample inside the training loop (custom transform + dataset + trainer), not as a materialized dataset; scene stats made spatially local; three bugs found along the way |
| [0018](0018-pin-segmentation-optimizer.md) | Pin the segmentation optimizer/`lr0`/`momentum`/`warmup_bias_lr` instead of `optimizer="auto"` — the `epochs` cap alone silently flipped MuSGD↔AdamW between the two arms of the copy-paste ablation, and `auto` also makes `args.yaml` misreport the run |
